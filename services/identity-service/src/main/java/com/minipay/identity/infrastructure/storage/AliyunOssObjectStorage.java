...
@Component
@ConditionalOnProperty(name = "minipay.identity.object-storage.provider", havingValue = "aliyun")
public final class AliyunOssObjectStorage implements ObjectStoragePort {
    private final OSS oss;
    private final String bucket;

    public AliyunOssObjectStorage(
            @Value("${minipay.identity.object-storage.endpoint}") String endpoint,
            @Value("${minipay.identity.object-storage.region}") String region,
            @Value("${minipay.identity.object-storage.bucket}") String bucket,
            @Value("${minipay.identity.object-storage.ram-role-name:}") String ramRoleName,
            @Value("${minipay.identity.object-storage.access-key-id:}") String accessKeyId,
            @Value("${minipay.identity.object-storage.access-key-secret:}") String accessKeySecret,
            @Value("${minipay.identity.object-storage.security-token:}") String securityToken) {
    ...
    public SignedUpload signUpload(
        ...
        try {
            ...
            throw new ProfileRejectedException("OBJECT_STORAGE_UNAVAILABLE", exception);
        }
    }
    ...
    public StoredObject head(String objectKey) {
        try {
            ...
            throw new ProfileRejectedException("AVATAR_OBJECT_UNAVAILABLE", exception);
        }
    }
