...
public class StoreOrderServiceImpl implements StoreOrderService {
    ...
    @Resource
    private ObjectProvider<MiniPayRefundApi> miniPayRefundApi;
...
//            }
    ...
    public void orderRefund(Long id, BigDecimal price, Integer type, Long salesId) {
        ...
        // MiniPay owns the wallet and ledger. Admin approval only writes an outbox request;
        // yshop is marked refunded after the signed refund-success callback.
        if ("minipay".equals(storeOrderDO.getPayType())) {
            if (storeOrderDO.getPayPrice().compareTo(price) != 0) {
                throw exception(new ErrorCode(999996, "MiniPay仅支持全额退款"));
            }
            MiniPayRefundApi gateway = miniPayRefundApi.getIfAvailable();
            if (gateway == null) {
                throw exception(new ErrorCode(999995, "MiniPay退款集成未启用"));
            }
            gateway.requestApprovedFullRefund(
                    storeOrderDO.getOrderId(), price, "yshop后台审核通过");
            return;
        }
