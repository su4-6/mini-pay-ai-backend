    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) {
        ...
        if (!SpringUtils.isProd()) {
            ApiAccessLog accessLog = handlerMethod == null
                    ? null : handlerMethod.getMethodAnnotation(ApiAccessLog.class);
            // MiniPay internal requests can contain full phone numbers and short-lived signed URLs.
            // Keep the URL and timing in local logs, but never serialize their query or body.
            boolean minipayInternalRequest = request.getRequestURI().startsWith("/internal/minipay/");
            boolean requestLoggingEnabled = !minipayInternalRequest
                    && (accessLog == null || accessLog.requestEnable());
            Map<String, String> queryString = requestLoggingEnabled
                    ? ServletUtils.getParamMap(request) : Map.of();
            String requestBody = requestLoggingEnabled && ServletUtils.isJsonRequest(request)
